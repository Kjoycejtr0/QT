...
uint8_t swd_flash_syscall_exec(const program_syscall_t *sysCallParam, uint32_t entry, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4)
{
    ...
    if (!swd_write_debug_state(&state)) {
			printf("Write State Faulf\r\n");
        return 0;
    }

    if (!swd_wait_until_halted()) {
			printf("swd_wait_until_halted Faulf\r\n");
        return 0;
    }

    if (!swd_read_core_register(0, &state.r[0])) {
			printf("swd_read_core_register Faulf\r\n");
        return 0;
    }
...
uint8_t swd_init_debug(void)
{
    ...
    if (!JTAG2SWD()) {
			printf("JTAG2SWD Fault\r\n");
        return 0;
    }
		
    if (!swd_write_dp(DP_ABORT, STKCMPCLR | STKERRCLR | WDERRCLR | ORUNERRCLR)) {
			printf("swd_write_dp Fault\r\n");
        return 0;
    }

    // Ensure CTRL/STAT register selected in DPBANKSEL
    if (!swd_write_dp(DP_SELECT, 0)) {
			printf("swd_write_dp DP_SELECT Fault\r\n");
        return 0;
    }

    // Power up
    if (!swd_write_dp(DP_CTRL_STAT, CSYSPWRUPREQ | CDBGPWRUPREQ)) {
			printf("swd_write_dp DP_CTRL_STAT Fault\r\n");
        return 0;
    }

    for (i = 0; i < timeout; i++) {
        if (!swd_read_dp(DP_CTRL_STAT, &tmp)) {
					printf("swd_read_dp DP_CTRL_STAT Fault\r\n");
            return 0;
        }
    ...
    if (!swd_write_dp(DP_SELECT, 0)) {
			printf("swd_write_dp DP_SELECT last Fault\r\n");
        return 0;
    }
    return 1;
}
        ...
        case RESET_PROGRAM:
            ...
            do{
                if (swd_read_word(DBG_HCSR, &val)==0)
								{
									printf("swd_read_word Fault 0x%.4X\r\n",val);
                  return 0;
                }
...
static uint8_t swd_write_data16(uint32_t address, uint16_t data)
{
    uint8_t tmp_in[4];
    uint8_t req, ack;
    // put addr in TAR register
...
uint8_t swd_write_half_word(uint32_t addr, uint16_t val)
{
    if (!swd_write_ap(AP_CSW, CSW_VALUE | CSW_SIZE16)) {
        return 0;
    }

    if (!swd_write_data16(addr, val)) {
        return 0;
    }
